...
for column in columns:
    ...
    train[column].replace(np.nan, 'NaN', inplace=True)
...
#litez: what does bd mean? seems to have -38 and -43 in the column.
columns = ['city', 'gender', 'registered_via']
for column in columns:
    members[column] = members[column].replace(np.nan, 'NaN')
    column_encoder = LabelEncoder()
